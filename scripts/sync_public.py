        ...
        with sync_playwright() as p:
            ...
            for _ in range(30):
                ...
                try:
                    ...
                    loc.first.click(timeout=5000);page.wait_for_timeout(900)
                except Exception:break
            text=page.locator('body').inner_text();browser.close();return text[:180000]
    except Exception as e:
        print('Playwright Amuse render failed:',repr(e));return fetch_text(AMUSE)
...
def response_text(instructions,user):
    ...
    return '\n'.join(c.get('text','') for item in data.get('output',[]) for c in item.get('content',[]) if c.get('type') in ('output_text','text') and c.get('text')).strip()

def parse_json(s):
    s=re.sub(r'^```(?:json)?\s*|\s*```$','',s.strip(),flags=re.S);m=re.search(r'\{.*\}',s,re.S)
...
def deterministic_information(text):
    ...
    if a<0:return []
    b=text.find('橘 めいの関連トピックス',a);section=text[a:b if b>0 else len(text)]
    lines=[x.strip() for x in section.splitlines() if x.strip()]
    cats={'Media','Live','Other','Release'};subtypes={'Event','Live','Stage','TV/WebTV','TV','Web','Radio','CM','Movie','Magazine','Book','Music','TICKET','PRESENT','GAME','Release','DVD/Blu-ray'}
    dates=[i for i,x in enumerate(lines) if re.fullmatch(r'20\d{2}年\s*\d{1,2}月\s*\d{1,2}日',x)];out=[]
    for n,i in enumerate(dates):
        prev=lines[max(0,i-3):i];typ=next((x for x in reversed(prev) if x in cats),'Other');subtype=next((x for x in reversed(prev) if x in subtypes and x!=typ),'')
        end=dates[n+1]-2 if n+1<len(dates) else len(lines);chunk=lines[i+1:max(i+2,end)]
        while chunk and chunk[-1] in cats|subtypes|{'もっと見る','ALL'}:chunk.pop()
        title=chunk[0] if chunk else '';body='\n'.join(chunk[1:]).strip();urls=re.findall(r'https?://[^\s）)]+',title+'\n'+body)
        out.append({'type':typ,'subtype':subtype,'date':jp_date_to_iso(lines[i]),'title_ja':title,'title_ko':'','body_ja':body,'body_ko':'','url':urls[0] if urls else AMUSE})
    seen=set();clean=[]
    for x in out:
        k=(x['date'],x['title_ja'])
...
def main():
    ...
    information=deterministic_information(amuse)
    if information:print('Amuse information items after expanding もっと見る:',len(information))
    if OPENAI_KEY:
        current=json.dumps({'fields':profile.get('fields',[]),'related_topics':profile.get('related_topics',[]),'upcoming':profile.get('upcoming',[])},ensure_ascii=False)
        instructions='''Maintain profile fields, related topics, and future schedule for voice actor 橘めい. Return one JSON object only with keys fields, related_topics, upcoming. Do NOT return or translate Amuse information; Information Korean text is manually curated elsewhere. upcoming items need kind (event/broadcast), date, date_label_ja,date_label_ko,title_ja,title_ko,detail_ja,detail_ko,url. Include only future items on or after TODAY. Use Eventernote for discovery/reference and prefer official URLs. Keep verified future broadcasts from CURRENT unless contradicted. Never invent dates, venues, URLs, roles, or events.'''
        data=parse_json(response_text(instructions,f'TODAY={today}\nCURRENT={current}\n\nAMUSE FULL EXPANDED:\n{amuse}\n\nEVENTERNOTE:\n{events}'))
        for k in ('fields','related_topics','upcoming'):
            if isinstance(data.get(k),list):profile[k]=data[k]
    if information:profile['information']=information
